...
class ClassifyByTopN(ClassifyByTarget):
    ...
    def target_top_n(self, tset, num=5, label=''):
        ...
        lst_key = list(frequency_sorted.keys())        
        
        ###check the tie at nth for frequency words
        if(num > len(lst_key) ):
            num = len(lst_key)
        else:    
            for ix in range(num, len(lst_key)):
                if( frequency_sorted[lst_key[num]][1] == frequency_sorted[lst_key[ix]][1] ):
                    num = ix
                else:
                    pass
                    
        word_top_n = lst_key[0:num]
