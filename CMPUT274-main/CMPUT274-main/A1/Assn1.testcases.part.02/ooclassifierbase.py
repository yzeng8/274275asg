...
class TrainingSet(C274):
    ...
    def copy(self):
        ''' 
        returns (after making a deepcopy) an object of class TrainingSet that contains the same attributes (e.g., training instances) as the original object of class TrainingSet. This copy() is similar to mylist.copy(), except that deepcopy (instead of a shallow copy) is used.
        '''
        ts = TrainingSet()
        ts.variable = copy.deepcopy(variable)

        for line in self.inObjList:
            ts.inObjList.append(copy.deepcopy(line))
        for ti in self.inObjHash:
            ts.inObjHash.append(copy.deepcopy(ti))
        return ts

    def add_training_set(self, tset):
        '''
        adds (via deep-copy) all the training instances of tset (which is of class TrainingSet) to an object of class TrainingSet.
        '''
        for k,v in tset.variable:
            if k not in self.variable.keys():
                self.variable[k] = v
